...
class TestCreateTodoItem(APITestCase):
  ...
  def test_item_has_correct_title(self):
    self.assertEqual(TodoItem.objects.get().title, 'Book flight ticket')
...
class TestUpdateTodoItem(APITestCase):
  ...
  def setUp(self):
    ...
    data = {'title': 'Book flight ticket', 'completed': True}
    self.response = self.client.put(url, data, format='json')

  def test_received_20_created_status_code(self):
